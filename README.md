...
The slide deck for this project's associated final presentation is included in the files list for this repository, accessible [here](https://github.com/briancmorrison/brian_data606/blob/main/DATA606%20-%20Final%20Presentation.pptx). Thank you for your interest in my project!
...
It may be the case, then, that a model designer is interested in a model that flags as many instances of fraud as possible without flagging legitimate transactions, to mitigate the time and resources committed to investigating “cold” leads. However, it may also be the case that a "wider net" approach is desirable for its ability to prompt users about potentially suspicious activity detected in their transactions, wherein a model with less precision could be tolerable as consumers have an active role in examining and dismissing legitimate transactions that are classified incorrectly. Based on this, unique data preparation and model structuring approaches may be seen as valuable depending on the business objectives available.

### Performance & Context

Aside from the finding that Stacking Classifiers modestly outperformed Deep Learning Models in novel classification tasks, another important finding was the discrepancy in performance among different class balancing techniques. As was discussed in the model evaluations, it appears that models trained on data whose classes were balanced using SMOTE slightly outperformed those trained on data whose classes were balanced using ROS - a finding that was not especially surprising given the analysis of fraud instances distribution across feature space performed with PCA. These models elicited higher F1 scores primarily as a result of their heightened precision, with a lower proclivity towards outputting "false flag" class predictions by labeling legitimate transactions as fraudulent. This could make models trained on data balanced using SMOTE - in particular, our higher performing Stacking Classifier - especially suited for business contexts in which resource constraints are a primary concern and investigations into transactions should be limited solely to transacions with a particularly high probability of being fraudulent, such as a system used by an internal fraud investigation team.

To contrast, models trained on data balanced using ROS produced lower F1 scores as a reuslt of less precise predictions - but importantly, also misclassified fraudulent transactions as legitimate at a lower rate than the models trained on SMOTE data. This could make these models more suited to suspicious activity alert systems, which require less immediate internal resource commitments thanks to the ability to request transaction confirmation from consumers. This business context could also spur some interest in a reinforcement learning approach to the problem, where models are tuned based on consumer indications of whether or not they were the ones making a transaction. 

In assessing model performance relative to the top-performing models on the [Kaggle competition leaderboard](https://www.kaggle.com/competitions/ieee-fraud-detection/leaderboard), it is clear that expansion of feature consideration and significant model tuning is necessary in order to produce a commercially viable, production-ready model. Top models produced combined F1 scores above 0.90, portraying highly robust fraud detection systems that, due to the dynamic nature of fraud, can even still be refined to enhance predictive validity. 
...
* *Feature Engineering* - The most immediate improvement that could be made to the current project is the inclusion of a wider group of features in the final training dataset, which would allow for models to discern more high-dimensional trends in the transactions. The decision to omit any columns containing null values, which was a significant majority of features, was primarily made on the basis of resource constraints, and almost certainly resulted in the removal of salient information for the models. More sophisticated methods to handle the presence of null values, such as replacing them with numeric constants (i.e. 0's) or merging them with populated fields from other columns, could assist in creating a more informative dataset. The topic of feature engineering in this context is discussed in more detail in [this excellent blog post](https://developer.nvidia.com/blog/leveraging-machine-learning-to-detect-fraud-tips-to-developing-a-winning-kaggle-solution/) published by the creators of the winning submisison from NVIDIA.
* *Hyperparameter Tuning* - Another significant improvement that could be made to this project would be using cross-fold validation to tune the hyperparameters of the models comprising the Stacking Classifier to ensure they are performing optimally. Identifying the ideal hyperparameters for simple classifiers by iteratively training and testing on partitions of the full dataset is a standard practice when building classification models, and was again primarily omitted based on resource constraints. Now that Stacking Classifiers have been identified as likely outperforming simple Deep Learning models in the present fraud detection task, while also being comparatively more time and resource efficient, it would be appropriate to revisit the selected weak classifiers and use a method such as GridSearchCV to identify optimal hyperparameters to use in a production-ready model. 
* *Transfer Learning* - Finally, revisiting the discussion around the significant role transfer learning plays in the Deep Learning ecosystem, exploring relevant pre-trained models that may elicit higher predictive validity in this use case could certainly be a way to improve the current project. While utilizing the performance of an extensively tuned model to compare with an untuned Stacking Classifier would not have been a productive way to gauge base model performance, this approach could be ideal in constructing a more robust model. 
